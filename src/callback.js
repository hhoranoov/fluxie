  ...
  if (helpTexts[data]) {
    ...
    if (taskIndex >= 0 && taskIndex < sortedTasks.length) {
      ...
      const inlineKeyboard = {
        inline_keyboard: [
          [{ text: toggleStatusText, callback_data: `toggle_${viewDay}_${taskIndex}` }, { text: '🗑️ Видалити', callback_data: `delete_${viewDay}_${taskIndex}` }],
    ...
    if (taskIndex >= 0 && taskIndex < sortedTasks.length) {
      ...
      await db.prepare('UPDATE tasks SET status = ?, completed_at = ?, last_completed_at = ? WHERE id = ?')
