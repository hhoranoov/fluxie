...
export async function handleTaskType(db, TELEGRAM_API_URL, chatId, dayArg, timeArg, task, taskType) {
  ...
  await db.prepare('INSERT INTO tasks (chat_id, day, time, task, status, type, week, completed_at, last_completed_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)')
    .bind(
      newTask.chat_id,
      newTask.day,
      newTask.time,
...
export async function handleViewTasks(db, TELEGRAM_API_URL, chatId, viewDay = 'today', messageId = null) {
  ...
  const inlineKeyboard = {
    inline_keyboard: [
      [{ text: 'Оновити', callback_data: `refresh_${viewDay}` }]
    ]
  };
  if (messageId) {
