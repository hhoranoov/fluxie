...
export async function handleCheckStreaks(db, TELEGRAM_API_URL, chatId) {
  ...
  for (const goalName in streaks) {
    ...
    streakMessages.push(
      `*Ціль:* \`${goalName}\`\n*Старт:* ${startDate.toLocaleDateString()}\n*Поточний streak:* \`${streak.streakCount} днів\` (_${streakDuration} днів без перерви_)`
    );
  }
...
export async function handleDeleteStreak(db, TELEGRAM_API_URL, chatId, goalName) {
  ...
  await sendMessage(
    TELEGRAM_API_URL,
    chatId,
    `✅ **Ціль \`${goalName}\` успішно видалена!**`,
    { parse_mode: 'Markdown' }
  );
}
...
export async function getUserStreaks(db, chatId) {
  const result = await db.prepare('SELECT streaks FROM user_streaks WHERE chat_id = ?')
    .bind(chatId)
    .first();
...
export async function saveUserStreaks(db, chatId, streaks) {
  await db.prepare('INSERT OR REPLACE INTO user_streaks (chat_id, streaks) VALUES (?, ?)')
    .bind(chatId, JSON.stringify(streaks))
    .run();
}
